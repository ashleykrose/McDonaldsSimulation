...
mean_order_time = 120.0  # Mean time of 2 minutes for customer to order
mean_food_prep_time = 300.0  # Mean time of 5 minutes for food prep
mean_payment_time = 120.0  # Mean time of 2 minutes for payment
mean_interarrival_time = 30.0  # Mean time of 1 minutes between customer arrivals
...
def customer_generator(env, run_time, order_station_1, order_station_2, pickup_window, number_of_customers, order_station_number):
    ...
    env.process(customer_generator(env, run_time, order_station_1, order_station_2, pickup_window, number_of_customers, order_station_number))


def customer(env, name, order_station_1, order_station_2, pickup_window, order_station_number):
    """Customer arrives, is served and leaves."""
    print('%s: Here I am' % name)
    order_station_1_count = order_station_1.count
    order_station_2_count = order_station_2.count
    order_line_1_count = len(order_station_1.queue)
    order_line_2_count = len(order_station_2.queue)
    ...
    if order_line_count < maximum_order_line:
        ...
        if left_food_prep_time > 0:
            wait_time = left_food_prep_time + payment_time
